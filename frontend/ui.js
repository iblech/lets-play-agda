...
// Code for setting up the dictionaries "characterDescriptions" and
// "typeSignatures" is generated by the build system and prepended to this
// source file.
...
function attachTooltip(link) {
  const parts = [];

  const href = link.getAttribute("href");
  if(href && typeSignatures[href]) {
    parts.push(typeSignatures[href]);
  }

  const d = characterDescriptions[link.innerText.replace(/^_/, "").replace(/_$/, "")];
  if(d !== undefined) {
    parts.push("Input method in Emacs:\n" + d);
  }
